...
function run() {
  ...
  if (enIndex) {
    ...
    tally(assert(enIndex.includes('images/og-default-v4.png'), 'en/index.html OG image v4 (social cache bust)'));
    ...
    tally(assert(!enIndex.includes('images/og-default-v3.png'), 'en/index.html has no stale og-default-v3.png meta URL'));
    tally(assert(enIndex.includes('property="og:title" content="Hiring loop with AI prompts"'),
...
const ROBOTS_META_FULL =
  '<meta name="robots" content="index, follow, max-snippet:-1, max-image-preview:large, max-video-preview:-1">';
const OG_IMAGE_ALT_LITERAL = 'Hiring loop in minutes with AI prompts for HR teams - Prompt Anatomy';
...
function assertGeoSurface(ctx) {
  ...
  if (ctx.sitemap) {
    ...
    tallyLocal(ctx.sitemap.includes('og-default-v4.png'), 'sitemap.xml: OG image in image:image');
