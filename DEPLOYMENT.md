...
| Stripe 500 `Fulfillment failed` on a non-PDF buy | Shared account: `.ceo` / other spoke Checkout hits this webhook. **200** `{ ignored: "unknown_product" }` only when `success_url` is another Prompt Anatomy host or metadata is clearly not a Hire kit. Do not `--resend` those as a Hire-kit PDF. Replay the event after deploy so Stripe stops retrying this endpoint. |
| Stripe 500 on a Hire-looking session | `.help` `success_url` (or `stripe.com` / missing host) + Price ID not in env + `metadata.product` not beginner\|advanced\|bundle = **500** `UNCONFIGURED_HIRE_PRODUCT`. Fix `STRIPE_PRICE_*` / Payment Link metadata — this is not a foreign SKU. |
