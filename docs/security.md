...
| Non-PDF checkout | **200** `{ ignored: "unknown_product" }` only with a positive other-spoke signal (non-Hire `metadata.product`, hub `metadata.plan` 3\|6, or `success_url` on `.ceo` / `.app` / `.pro` / `.info` / `.cloud` / `.site` / `.blog`). Hire Price ID or `metadata.product` beginner\|advanced\|bundle still fulfills. `.help` / unknown host + unmatched price → **500** `UNCONFIGURED_HIRE_PRODUCT` (Stripe retries). Classifier runs before Redis lock. |
...
- [llms.txt](../llms.txt) ir [llms-full.txt](../llms-full.txt) — AI-friendly site map (Training hub → `.app`, Optional legal). Google Search ignores `llms.txt`. v2: `rel="describedby"` + spoke `/en/**/index.md` twins.
