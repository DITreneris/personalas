...
| **OG paveikslėlis** | Vienas bendras **`images/og-default-v4.png`** (`OG_IMAGE_REL` build skripte) – **anglų** tekstas iš `marketing.seo.ogImage` (generuoti: `npm run generate:og`). Keičiant social cache – naujas failo vardas, ne tas pats URL. Ant PNG – outcome žinutė ir veiksmas, ne kaina. |
